...
use aqua_swap_shared::{errors::AquaSwapError, interfaces::PairClient};
...
impl FactoryContract {
    ...
    pub fn create_pair(env: Env, token_a: Address, token_b: Address) -> Address {
        ...
        if token_a == token_b {
            env.panic_with_error(AquaSwapError::IdenticalAddresses);
        }
        if storage::get_pair(&env, &token_a, &token_b).is_some() {
            env.panic_with_error(AquaSwapError::PairAlreadyExists);
        }
        ...
        let prefix = Bytes::from_slice(&env, b"aquaswap_pair_v1_");
    ...
    pub fn get_pair(env: Env, token_a: Address, token_b: Address) -> Address {
        match storage::get_pair(&env, &token_a, &token_b) {
            Some(addr) => addr,
            None => env.panic_with_error(AquaSwapError::PairNotFound),
        }
    }
    ...
    pub fn all_pairs(env: Env, index: u32) -> Address {
        match storage::get_pair_at(&env, index) {
            Some(addr) => addr,
            None => env.panic_with_error(AquaSwapError::PairIndexOutOfBounds),
        }
    }
