...
use aqua_swap_shared::{
    errors::AquaSwapError,
...
impl PairContract {
    ...
    pub fn initialize(env: Env, token_x: Address, token_y: Address, factory: Address) {
        if env.storage().instance().has(&DataKey::Initialized) {
            env.panic_with_error(AquaSwapError::AlreadyInitialized);
        }
    ...
    pub fn add_liquidity(
        ...
        let (amount_x, amount_y) = if reserve_x == 0 && reserve_y == 0 {
            ...
            if y_opt <= amount_y_desired {
                if y_opt < amount_y_min {
                    env.panic_with_error(AquaSwapError::InsufficientBAmount);
                }
                (amount_x_desired, y_opt)
            } else {
                let x_opt = math::quote(&env, amount_y_desired, reserve_y, reserve_x);
                if x_opt > amount_x_desired {
                    env.panic_with_error(AquaSwapError::InsufficientAAmount);
                }
        ...
        let lp_minted = if total_supply == 0 {
            ...
            if lp <= 0 {
                env.panic_with_error(AquaSwapError::InsufficientLiquidityMinted);
            }
    ...
    pub fn remove_liquidity(
        ...
        if liquidity <= 0 {
            env.panic_with_error(AquaSwapError::InsufficientLiquidity);
        }
        ...
        if amount_x <= 0 || amount_y <= 0 {
            env.panic_with_error(AquaSwapError::InsufficientLiquidityBurned);
        }
    ...
    pub fn swap(env: Env, caller: Address, amount_x_out: i128, amount_y_out: i128, to: Address) {
        ...
        if amount_x_out <= 0 && amount_y_out <= 0 {
            env.panic_with_error(AquaSwapError::InsufficientOutputAmount);
        }
        if amount_x_out > 0 && amount_y_out > 0 {
            env.panic_with_error(AquaSwapError::InsufficientOutputAmount);
        }
        ...
        if amount_x_out >= reserve_x {
            env.panic_with_error(AquaSwapError::InsufficientReserve);
        }
        if amount_y_out >= reserve_y {
            env.panic_with_error(AquaSwapError::InsufficientReserve);
        }
        if to == token_x || to == token_y {
            env.panic_with_error(AquaSwapError::InvalidTo);
        }
        ...
        if amount_x_in <= 0 && amount_y_in <= 0 {
            env.panic_with_error(AquaSwapError::InsufficientInputAmount);
        }
        ...
        if k_new < k_old {
            env.panic_with_error(AquaSwapError::InvariantViolation);
        }
    ...
    pub fn lp_name(env: Env) -> String {
        String::from_str(&env, "AquaSwap LP Token")
    }
