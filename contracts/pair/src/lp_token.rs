...
pub fn burn(env: &Env, from: &Address, amount: i128) {
    ...
    if bal < amount {
        env.panic_with_error(AquaSwapError::InsufficientBalance);
    }
...
pub fn transfer_from(env: &Env, spender: &Address, from: &Address, to: &Address, amount: i128) {
    ...
    if exp < env.ledger().sequence() {
        env.panic_with_error(AquaSwapError::AllowanceExpired);
    }
    if allowed < amount {
        env.panic_with_error(AquaSwapError::InsufficientAllowance);
    }
