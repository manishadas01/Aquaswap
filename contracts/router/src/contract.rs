...
impl RouterContract {
    ...
    pub fn remove_liquidity(
        ...
        // pair.remove_liquidity burns LP directly from `caller` — no separate
        // transfer-to-pair step (that would double-spend the same LP balance:
        // once via this transfer, again via the burn inside remove_liquidity).
        let token_x = pair.token_x();
    ...
    pub fn swap_exact_tokens_for_tokens(
        ...
        if path.len() < 2 {
            env.panic_with_error(AquaSwapError::InvalidPath);
        }
        let factory_addr: Address = env.storage().instance().get(&DataKey::Factory).unwrap();
    ...
    pub fn swap_tokens_for_exact_tokens(
        ...
        if required_in > amount_in_max {
            env.panic_with_error(AquaSwapError::ExcessiveInputAmount);
        }
    ...
    fn check_deadline(env: &Env, deadline: u64) {
        if env.ledger().timestamp() > deadline {
            env.panic_with_error(AquaSwapError::ExpiredDeadline);
        }
    }
