...
use aqua_swap_shared::errors::AquaSwapError;
...
impl TokenContract {
    ...
    pub fn burn(env: Env, from: Address, amount: i128) {
        ...
        let bal: i128 = env.storage().persistent().get(&key).unwrap_or(0);
        if bal < amount {
            panic_with_error!(&env, AquaSwapError::InsufficientBalance);
        }
        env.storage().persistent().set(&key, &(bal - amount));
        storage::extend_persistent(&env, &key);
    ...
    fn do_spend_allowance(env: &Env, from: &Address, spender: &Address, amount: i128) {
        ...
        let exp: u32 = env.storage().persistent().get(&exp_key).unwrap_or(0);
        if exp < env.ledger().sequence() {
            panic_with_error!(env, AquaSwapError::AllowanceExpired);
        }
        let key = DataKey::Allowance(from.clone(), spender.clone());
        let allowed: i128 = env.storage().persistent().get(&key).unwrap_or(0);
        if allowed < amount {
            panic_with_error!(env, AquaSwapError::InsufficientAllowance);
        }
