...
AquaSwap is a three-layer system:
...
│ • Not used in AquaSwap (all state is persistent)     │
└─────────────────────────────────────────────────────────┘
```
...
if amount_in <= 0 {
    panic_with_error!(&env, AquaSwapError::InsufficientInputAmount);
}
```
