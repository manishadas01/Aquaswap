...
AquaSwap deploys three Soroban smart contracts to the Stellar network:
...
#   aqua_swap_factory.wasm
#   aqua_swap_pair.wasm
#   aqua_swap_router.wasm
#   aqua_swap_token.wasm
```
...
stellar contract optimize \
  --wasm target/wasm32v1-none/release/aqua_swap_pair.wasm
# Creates: aqua_swap_pair.optimized.wasm
```
...
PAIR_WASM_HASH=$(stellar contract upload \
  --network testnet \
  --source deployer \
  --wasm target/wasm32v1-none/release/aqua_swap_pair.optimized.wasm)
...
FACTORY_ADDRESS=$(stellar contract deploy \
  --network testnet \
  --source deployer \
  --wasm target/wasm32v1-none/release/aqua_swap_factory.optimized.wasm)
...
ROUTER_ADDRESS=$(stellar contract deploy \
  --network testnet \
  --source deployer \
  --wasm target/wasm32v1-none/release/aqua_swap_router.optimized.wasm)
...
TOKEN_A_ADDRESS=$(stellar contract deploy \
  --network testnet \
  --source deployer \
  --wasm target/wasm32v1-none/release/aqua_swap_token.optimized.wasm)
...
aquaswap_indexer_lag_ledgers           # How far behind current ledger
aquaswap_events_processed_total        # Events processed (counter)
aquaswap_pairs_indexed_total           # Total pairs indexed

# Protocol metrics (from indexed data)
aquaswap_tvl_usd                       # Total Value Locked
aquaswap_volume_24h_usd                # 24-hour volume
aquaswap_swaps_24h_count               # Number of swaps
aquaswap_active_pairs_count            # Pairs with > 0 TVL
```
  ...
  - name: aquaswap
    rules:
      - alert: IndexerLagging
        expr: aquaswap_indexer_lag_ledgers > 100
        for: 5m
        labels:
          severity: warning
        annotations:
      ...
      - alert: IndexerDown
        expr: up{job="aquaswap-indexer"} == 0
        for: 2m
        labels:
          severity: critical
        annotations:
