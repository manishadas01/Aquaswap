# AquaSwap — Product Requirements Document (PRD)
...
AquaSwap is a decentralized exchange (DEX) protocol built on the Stellar blockchain using Soroban smart contracts. It implements an Automated Market Maker (AMM) model inspired by Uniswap V2, redesigned from first principles for Stellar's execution environment, account model, and asset system.
...
**Why now?** Soroban is production-ready but the DeFi ecosystem is nascent. AquaSwap is positioned to be foundational liquidity infrastructure for the Stellar DeFi ecosystem.
...
**Why they'd choose AquaSwap**: Low fees to add/remove liquidity (Stellar's near-zero fees), transparent on-chain fee accrual, simple UI.
...
**Why they'd choose AquaSwap**: No KYC, no custody risk, near-instant finality, predictable pricing.
...
**Why they'd choose AquaSwap**: Clean SDK, well-documented contracts, composable architecture.

#### Arbitrageurs
Algorithmic traders who profit by correcting price imbalances between AquaSwap pools and other markets. They provide a critical economic function: keeping pool prices aligned with global market prices.
...
AquaSwap fills the infrastructure gap: a permissionless, composable, fee-generating AMM on Stellar that serves as foundational DeFi rails for the ecosystem.
