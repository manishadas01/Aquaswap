...
AquaSwap adopts a **defense-in-depth** security posture. No single mitigation is considered sufficient. Multiple independent layers of protection are applied to all critical paths.
...
if reserve_in == 0 || reserve_out == 0 {
    panic_with_error!(&env, AquaSwapError::InsufficientLiquidity);
}
...
if balance_x_adj.checked_mul(balance_y_adj).unwrap()
    ...
    .unwrap()
{
    panic_with_error!(&env, AquaSwapError::InvariantViolation);
}
```
...
fn initialize(env: Env, token_x: Address, token_y: Address, factory: Address) {
    if env.storage().instance().has(&DataKey::Initialized) {
        panic_with_error!(&env, AquaSwapError::AlreadyInitialized);
    }
...
fn ensure_deadline(env: &Env, deadline: u64) {
    if env.ledger().timestamp() > deadline {
        panic_with_error!(env, AquaSwapError::ExpiredDeadline);
    }
}
...
**AquaSwap Mitigations**:
- AquaSwap does NOT expose a spot price oracle function (no `get_price()` in MVP)
...
**AquaSwap Response**: Document IL prominently in frontend and LP documentation. LPs earn fees that partially offset IL.
...
The following are explicit security assumptions. If any are violated, the security properties of AquaSwap may not hold.
