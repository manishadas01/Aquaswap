#!/usr/bin/env bash
# AquaSwap — Real Token Deployment Script (Testnet)
#
...
echo "======================================================"
echo " AquaSwap Real Token Deployment"
...
cargo build \
  --package aqua-swap-factory \
  --package aqua-swap-pair \
  --package aqua-swap-router \
...
PAIR_WASM_HASH=$(stellar contract upload \
  --network "$NETWORK" \
  --source "$DEPLOYER_KEY" \
  --wasm target/wasm32v1-none/release/aqua_swap_pair.wasm 2>&1 | tail -1)
echo "  Pair WASM hash: $PAIR_WASM_HASH"

FACTORY=$(stellar contract deploy \
  --network "$NETWORK" \
  --source "$DEPLOYER_KEY" \
  --wasm target/wasm32v1-none/release/aqua_swap_factory.wasm \
...
ROUTER=$(stellar contract deploy \
  --network "$NETWORK" \
  --source "$DEPLOYER_KEY" \
  --wasm target/wasm32v1-none/release/aqua_swap_router.wasm \
  2>&1 | grep -v "^ℹ️\|^🌎\|^Simul\|^Sign" | tail -1)
echo "  Router: $ROUTER"
