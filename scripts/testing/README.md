...
wallets and drives each one through the full AquaSwap user journey against the
...
- **Secret keys are written outside the repository** to
  `~/.aquaswap/test-wallets.json` (mode `0600`), and the patterns
  `*test-wallets*.json` / `.aquaswap/` are gitignored.
