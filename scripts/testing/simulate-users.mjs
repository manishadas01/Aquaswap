#!/usr/bin/env node
/**
 * AquaSwap — Real-User Simulation Harness
 * ------------------------------------------------------------------------------
 * Generates a cohort of independent testnet wallets and drives each one through
 * the full AquaSwap user journey against the LIVE deployed contracts:
 *
 ...
 *   - Wallet secret keys are written to  ~/.aquaswap/test-wallets.json
...
const WALLET_STORE =
  process.env.WALLET_STORE || join(homedir(), '.aquaswap', 'test-wallets.json');
...
// ── Main ────────────────────────────────────────────────────────────────────────
async function main() {
  log(`\nAquaSwap real-user simulation — ${USERS} users on Stellar Testnet`);
  log(`Router: ${ROUTER}\n`);
